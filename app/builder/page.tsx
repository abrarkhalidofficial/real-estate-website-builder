...
export default function BuilderPage() {
  ...
  const convertPuckToPageData = (puck: any, existing: PageData): PageData => {
    const content: Array<{ type: string; props: any }> = Array.isArray(
      puck?.content
    )
      ? puck.content
      : [];
          ...
          case "FAQ":
            return {
              ...
              items: Array.isArray(item.props?.items)
                ...
                : existing.sections.find((s) => s.id === id && s.type === "faq")
                    ?.items ?? [],
            };
          case "Testimonials":
            return {
  ...
  if (!pageData) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-primary dark:text-neutral-50 mb-2">
            Template not found
          </h1>
          <p className="text-neutral-600 dark:text-neutral-400">
            Please select a valid template to get started.
          </p>
        </div>
      </div>
    );
